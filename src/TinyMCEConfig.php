...
/**
 * Default configuration for HTMLEditor specific to hugerte
 */
class HugeRTEConfig extends HTMLEditorConfig implements i18nEntityProvider
{
    /**
     * Map of locales to hugerte's supported languages
     *
     * @link https://www.tiny.cloud/get-tiny/language-packages/
     */
    private static array $hugerte_lang = [
        'ar_EG' => 'ar',
        'az_AZ' => 'az',
        'bg_BG' => 'bg_BG',
        'ca_AD' => 'ca',
     ...
     */
    private static string $base_dir = 'guysartorelli/silverstripe-htmleditor-hugerte:client/dist/hugerte';

    /**
     * Location of hugerte translation files relative to BASE_DIR.
     *
     * Supports vendor/module:path
     */
    private static string $lang_dir = 'guysartorelli/silverstripe-htmleditor-hugerte:client/hugerte_lang';
    ...
    private static array $editor_css = [
        'guysartorelli/silverstripe-htmleditor-hugerte:client/dist/styles/editor.css',
    ];
     ...
     * * `i18n` to store a translation key (e.g.: `HugeRTEConfig.BESTFIT`)
    ...
    private static array $image_size_presets = [
        [
            'width' => 600,
            'i18n' => HugeRTEConfig::class . '.BEST_FIT',
            'text' => 'Best fit',
            'name' => 'bestfit',
            'default' => true,
        ],
        [
            'i18n' => HugeRTEConfig::class . '.ORIGINAL',
            'text' => 'Original',
            'name' => 'originalsize',
        ],
    ];
    ...
    /**
     * Default HugeRTE JS options which apply to all new configurations.
     *
    ...
    protected static string $configType = 'hugeRTE';

    protected static string $schemaComponent = 'HugeRteHtmlEditorField';
    ...
    /**
     * Name of the HugeRTE theme to use
     */
                ...
                if ($attrData) {
                    foreach (explode('|', $attrData ?? '') as $attr) {
                        if (preg_match($attrRuleRegExp ?? '', $attr ?? '', $matches)) {
                            ...
                            // This allows attributes like "xml:lang" and matches how HugeRTE's js handles this.
                            ...
                            // Denied from global
                            // (note it's unclear what this means, but this is done by HugeRTE js as well)
                            if ($attrType === '-') {
     ...
     * the plugin will be loaded by hugerte.PluginManager.load() instead of through hugeRTE.init().
     ...
     *
     * If passing in a non-associative array, the plugin name should be located in the standard hugerte
     * plugins folder.
     *
    ...
    /**
     * Generate the JavaScript that will set HugeRTE's configuration:
     * - Parse all configurations into JSON objects to be used in JavaScript
     * - Includes HugeRTE and configurations using the {@link Requirements} system
     */
    protected function getConfig(): array
    {
        ...
        $baseResource = $this->getHugeRTEResource();
        if ($baseResource instanceof ModuleResource) {
            $hugeRTEBaseURL = $baseResource->getURL();
        } else {
            $hugeRTEBaseURL = Controller::join_links(Director::baseURL(), $baseResource);
        }
        $settings['baseURL'] = $hugeRTEBaseURL;
        ...
        if (!Director::is_absolute_url($theme)) {
            $theme = Controller::join_links($hugeRTEBaseURL, "themes/{$theme}/theme.min.js");
        }
        ...
        $settings['language'] ??= HugeRTEConfig::getHugerteLang();
        $langUrl = HugeRTEConfig::getHugerteLangUrl();
        if ($langUrl) {
    ...
    private function initImageSizePresets(array &$settings): void
    {
        ...
        foreach ($settings['image_size_presets'] as &$preset) {
            ...
            if (isset($preset['i18n'])) {
                ...
                $preset['text'] = _t(
                    HugeRTEConfig::class . '.PIXEL_WIDTH',
                    '{width} pixels',
                    $preset
                );
            }
    ...
    /**
     * Generate gzipped HugeRTE configuration including plugins and languages.
     * This ends up "pre-loading" HugeRTE bundled with the required plugins
     * so that multiple HTTP requests on the client don't need to be made.
     */
    public function getScriptURL(): string
    {
        $generator = Injector::inst()->get(HugeRTEScriptGenerator::class);
        return $generator->getScriptURL($this);
    }

    public function init(): void
    {
        // include HugeRTE Javascript
    ...
    /**
     * Get the current hugeRTE language
     */
    public static function getHugerteLang(): string
    {
        $lang = static::config()->get('hugerte_lang');
     ...
     */
    public static function getHugerteLangUrl(): string
    {
        $lang = static::getHugerteLang();
    ...
    /**
     * Returns the full filesystem path to HugeRTE resources (which could be different from the original hugerte
     * location in the module).
     *
     * Path will be absolute.
     */
    public function getHugeRTEResourcePath(): string
    {
        $resource = $this->getHugeRTEResource();
    ...
    /**
     * Get front-end url to hugerte resources
     */
    public function getHugeRTEResourceURL(): string
    {
        $resource = $this->getHugeRTEResource();
        if ($resource instanceof ModuleResource) {
    ...
    /**
     * Get resource root for HugeRTE, either as a string or ModuleResource instance
     * Path will be relative to BASE_PATH if string.
     *
     * @throws Exception
     */
    public function getHugeRTEResource(): ModuleResource|string
    {
        ...
        throw new Exception(sprintf(
            'If the silverstripe/admin module is not installed you must set the HugeRTE path in %s.base_dir',
            __CLASS__
        ));
    }
     ...
     * @return $this
     */
    public function setFolderName(string $folderName): HugeRTEConfig
    {
    ...
    {
        $entities = [
            HugeRTEConfig::class . '.PIXEL_WIDTH' => '{width} pixels',
        ];
