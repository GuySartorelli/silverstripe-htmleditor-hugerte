...
/**
 * Generates hugerte config using a combined file generated via a standard
 * SilverStripe {@link GeneratedAssetHandler}
 */
class HugeRTECombinedGenerator implements HugeRTEScriptGenerator, Flushable
{
    use Configurable;
    use Injectable;
     ...
     * @var string
     */
    private static $filename_base = '_hugerte/hugerte-{name}-{hash}.js';
    ...
    /**
     * Build raw config for hugerte
     *
     * @param HugeRTEConfig $config
     * @return string
     */
    public function generateContent(HugeRTEConfig $config)
    {
        $hugerteDir = $config->getHugeRTEResource();
        ...
        if ($language) {
            $files[] = $this->resolveRelativeResource($hugerteDir, "langs/{$language}");
        }
        ...
        foreach ($config->getPlugins() as $plugin => $path) {
            ...
            // Core hugerte plugin
            $files[] = $this->resolveRelativeResource($hugerteDir, "plugins/{$plugin}/plugin");
            if ($language) {
                $files[] = $this->resolveRelativeResource($hugerteDir, "plugins/{$plugin}/langs/{$language}");
            }
        }
        ...
        if ($theme) {
            $files[] = $this->resolveRelativeResource($hugerteDir, "themes/{$theme}/theme");
            if ($language) {
                $files[] = $this->resolveRelativeResource($hugerteDir, "themes/{$theme}/langs/{$language}");
            }
        }

        // Process source files
        $files = array_filter($files ?? []);
        $libResource = $this->resolveRelativeResource($hugerteDir, 'hugerte');
...
SCRIPT;
        ...
        // Load all hugerte script files into buffer
        ...
        $buffer[] = "window.hugerte.each('$filesList'.split(','),"
            . "function(f){hugerte.ScriptLoader.markDone(baseURL+f);});";
     ...
     * @return string
     */
    protected function checkName(HugeRTEConfig $config)
    {
     ...
     * @return mixed
     */
    public function generateFilename(HugeRTEConfig $config)
    {
