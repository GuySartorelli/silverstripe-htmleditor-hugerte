...
class HugeRTEConfigTest extends SapphireTest
{
    public function testEditorIdentifier()
    {
        $config = HugeRTEConfig::get('myconfig');
        $this->assertEquals('myconfig', $config->getOption('editorIdentifier'));
    }

    /**
     * Ensure that all HugeRTEConfig.hugerte_lang are valid
     */
    public function testLanguagesValid()
    {
        $configDir = HugeRTEConfig::config()->get('lang_dir');
        if (!$configDir) {
            $this->markTestSkipped('Test skipped without HugeRTE language resource folder being installed');
        }
        ...
        $checked = [];
        foreach (HugeRTEConfig::config()->get('hugerte_lang') as $locale => $resource) {
    ...
    {
        HugeRTEConfig::config()->set('editor_css', [
            'silverstripe/framework:tests/php/Forms/HTMLEditor.css'
        ]);

        // Test default config
        $config = new HugeRTEConfig();
    ...
    {
        HugeRTEConfig::config()->set('image_size_presets', [
            ['i18n' => HugeRTEConfig::class . '.TEST', 'text' => 'Foo bar'],
            ['text' => 'No translation key'],
            ['i18n' => HugeRTEConfig::class . '.NO_TRANSLATION_TEXT'],
            ['i18n' => HugeRTEConfig::class . '.TEST_TWO', 'text' => 'Bar foo'],
        ]);

        $config = HugeRTEConfig::create();
        ...
        $this->assertEquals('Foo bar', $translations[HugeRTEConfig::class . '.TEST']);
        $this->assertEquals('Bar foo', $translations[HugeRTEConfig::class . '.TEST_TWO']);
        $this->assertEquals('{width} pixels', $translations[HugeRTEConfig::class . '.PIXEL_WIDTH']);
    }
    ...
    public function testEnablePluginsByArrayWithPaths()
    {
        ...
        // Get the path dynamically, because CI uses _resources/client/dist/hugerte/plugins when run as nthe root.
        $module = ModuleLoader::getModule('guysartorelli/silverstripe-htmleditor-hugerte');
        $pluginsPath = $module->getResource('client/dist/hugerte/plugins/')->getURL();
    ...
    {
        HugeRTEConfig::config()->remove('base_dir');
    ...
    public function testGetAttributes()
    {
        ...
        HugeRTEConfig::config()->set('fixed_row_height', 0);
        ...
        HugeRTEConfig::config()->set('fixed_row_height', 20);
        // Set the rows to 0
        $editor->setRows(0);
        // Get the attributes and config from the editor
        $attributes = $editor->getAttributes();
