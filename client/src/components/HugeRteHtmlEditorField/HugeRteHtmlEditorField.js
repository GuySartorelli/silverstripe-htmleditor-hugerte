...
class HugeRteHtmlEditorField extends TextField {
  /**
   * sets initial state:
  ...
  getEditor() {
    return window.HugeRTE && window.HugeRTE.get(this.getInputProps().id);
  }
   ...
   */
  handleReady() {
    if (!window.HugeRTE && window.hugerte) {
  ...
  /**
   * HugeRTE operates from a global script being loaded in first.
  ...
  componentWillUnmount() {
    ...
    const editorElement = this.getEditorElement();
    // Tell hugeRTE to persist changes into the text field
    ...
    // This is pretty awful because HugeRTE triggers jQuery events which aren't picked up
...
export { HugeRteHtmlEditorField as Component };

export default fieldHolder(HugeRteHtmlEditorField);
