...
// eslint-disable-next-line import/no-unresolved -- webpack external from silverstripe/asset-admin
...
// eslint-disable-next-line import/no-unresolved -- webpack external from silverstripe/asset-admin
import * as modalActions from 'state/modal/AssetAdminModalActions';
import { imageSizePresetButtons } from './HugeRTE_ssmedia_sizepressets';
...
(() => {
  const ssmedia = {
     ...
     *
     * @param {Object} editor HugeRTE editor object
     */
    init(editor) {
      const insertTitle = i18n._t('HugeRTE.INSERT_FROM_FILES', 'Insert from Files');
      const editTitle = i18n._t('HugeRTE.EDIT_IMAGE', 'Edit image');
      const deleteTitle = i18n._t('HugeRTE.DELETE_IMAGE', 'Delete image');
      const contextTitle = i18n._t('HugeRTE.FILE', 'File');
  ...
  hugerte.PluginManager.add('ssmedia', (editor) => ssmedia.init(editor));
})();
...
jQuery.entwine('ss', ($) => {
  $('.js-injector-boot #insert-media-react__dialog-wrapper').entwine({
    ...
    open() {
      ...
      const imageSizePresets = hugerte.activeEditor.getParam('image_size_presets');
