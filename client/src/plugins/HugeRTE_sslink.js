/* global hugerte, navigator */
import HugeRTEActionRegistrar from 'lib/HugeRTEActionRegistrar';
...
const plugin = {
  ...
  init(editor) {
    function getActions() {
      ...
      return HugeRTEActionRegistrar.getSortedActions('sslink', editor.getParam('editorIdentifier'), true)
    ...
    const title = i18n._t('HugeRTE.INSERT_LINK', 'Insert link');
    const titleWithShortcut = i18n.inject(
      i18n._t('HugeRTE.INSERT_LINK_WITH_SHORTCUT', 'Insert link {shortcut}'),
      { shortcut: `[${metaKey}+K]` }
    );
    ...
    function openLinkDialog() {
      ...
      const linkNode = jQuery(hugerte.activeEditor.selection.getNode()).closest('a');
      hugerte.activeEditor.selection.select(linkNode[0]);
      const node = hugerte.activeEditor.selection.getNode();
      const href = node.getAttribute('href');

      if (href) {
        editor.execCommand(HugeRTEActionRegistrar.getEditorCommandFromUrl(href));
      }
    }
    ...
    editor.ui.registry.addButton('sslink-edit', {
      text: i18n._t('HugeRTE.EDIT_LINK', 'Edit link'),
      onAction: openLinkDialog,
    });
    editor.ui.registry.addButton('sslink-remove', {
      text: i18n._t('HugeRTE.REMOVE_LINK', 'Remove link'),
  ...
  handleRemoveLinkClick(editor) {
    ...
    // Merge adjacent textNodes after removing <a> nodes so that hugerte.js can properly
...
jQuery.entwine('ss', ($) => {
  $('.insert-link__dialog-wrapper').entwine({
    ...
    /**
     * @param selection The current hugeRTE selection for this WYSIWYG
     * @return {Boolean}
     */
    linkCanWrapSelection(editor, selection) {
      ...
      if (this.checkNodeMatches(node, selection.getSel().focusNode) && this.checkNodeMatches(node, selection.getSel().anchorNode)) {
        const parsed = hugerte.activeEditor.dom.createFragment(`<a>${node.outerHTML}</a>`);
...
// Adds the plugin class to the list of available HugeRTE plugins
hugerte.PluginManager.add('sslink', (editor) => plugin.init(editor));
