 ...
 */
ss.editorWrappers.hugeRTE = (function () {
  ...
  return {
     ...
     */
    destroy() {
      hugerte.EditorManager.execCommand('mceRemoveEditor', false, editorID);
    },

    /**
     * Get HugeRTE Editor instance
     *
     * @returns Editor
     */
    getInstance() {
      return hugerte.EditorManager.get(editorID);
    },
    ...
    prepValueForChangeTracker(value) {
      ...
      const serializer = hugerte.html.Serializer(config);
      const parser = hugerte.html.DomParser(config);
    ...
    cleanLink(href, node) {
      ...
      const cu = hugeRTE.settings.convert_urls;
      // eslint-disable-next-line no-eval
      if (cb) href = eval(`${cb}(href, node, true);`);

      // Turn into relative, if set in HugeRTE config
      if (cu && href.match(new RegExp(`^${escapeRegExp(hugeRTE.settings.document_base_url)}(.*)$`))) {
        href = RegExp.$1;
      }

      // Get rid of HugeRTE's temporary URLs
...
ss.editorWrappers.default = ss.editorWrappers.hugeRTE; // @TODO need to do this per field, not globally
